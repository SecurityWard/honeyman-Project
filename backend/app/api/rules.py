...
def _rules_dir() -> Path:
    """Resolve the rules source directory.

    Override with RULES_DIR in the backend's .env. Default assumes the
    repo's standard layout — backend/ and agent/ as siblings at the
    repo root — and points at agent/rules so there is exactly one
    source of truth for the rule manifest served to sensors.
    """
    configured = getattr(settings, "RULES_DIR", None)
    if configured:
        return Path(configured)
    # backend/app/api/rules.py -> parents[3] is the repo root.
    return Path(__file__).resolve().parents[3] / "agent" / "rules"
