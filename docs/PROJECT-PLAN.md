...
| Nginx + Let's Encrypt | ✅ Deployed — config snapshot in `deployment/nginx/honeyman.conf` |
...
(`deployment/phase_a_apply.sh`) ran successfully against the
...
deployment/              nginx, install.sh, ops timers, DEPLOY.md
data/                    Malware-hash DB shipped to sensors
```
