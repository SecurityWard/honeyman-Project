...
After cloning you should have `/root/honeyman-Project/backend/` and the matching `frontend/` directory.
...
```bash
cd /root/honeyman-Project/backend
python3 -m venv venv
...
```bash
cd /root/honeyman-Project/frontend
...
Type=simple
User=root
WorkingDirectory=/root/honeyman-Project/backend
ExecStart=/root/honeyman-Project/backend/venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8000 --workers 4
EnvironmentFile=/root/honeyman-Project/backend/.env
Restart=on-failure
RestartSec=5
